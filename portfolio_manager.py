...
class PortfolioManager:
    ...
    def load(self):
        ...
        if data is None:
            ...
            log.info(f"Portfolio geladen: {len(self.positions)} Positionen, {format_currency(self.cash)} Cash.")
    ...
    def _load_from_alpaca(self):
        try:
            ...
            log.info(f"[BROKER SYNC] Portfolio von Alpaca geladen: {len(self.positions)} Positionen, {format_currency(self.cash)} Cash.")
            self.save()
        except Exception as e:
    ...
    def get_trailing_stop_triggers(self, market_data: Dict[str, Dict], trail_pct: float = 0.12) -> List[Dict]:
        trailing_sells = []
        for ticker, pos in self.positions.items():
    ...
    def stale_position_flag(self, stale_gain_low: float = -0.02, stale_gain_high: float = 0.02, min_hold_days: int = 15) -> Dict[str, Dict]:
        stale_flags = {}
        today = datetime.now()
        for ticker, pos in self.positions.items():
            ...
            stale_flags[ticker] = {"stale": is_stale, "pnl_pct": round(pnl_pct * 100, 2), "hold_days": hold_days}
        return stale_flags
    ...
    def print_summary(self):
        ...
        log.info(f"  P&L:         {format_currency(total - self.initial_capital)} ({pct_change(self.initial_capital, total):+.2f}%)")
        log.info(f"  Positionen:  {len(self.positions)}")
        for ticker, pos in self.positions.items():
            alloc = pos.get("market_value", 0) / total * 100 if total else 0
            log.info(f"    {ticker:<6} {pos.get('quantity', 0):.4f} Stk @ ${pos.get('avg_price', 0):.2f} -> {format_currency(pos.get('market_value', 0))} | ({alloc:.1f}%) | P&L: {format_currency(pos.get('unrealized_pnl', 0))}")
        log.info("=" * 60)
    ...
    def apply_trade(self, ticker: str, action: str, quantity: float, price: float):
        ...
        if action == "BUY":
            ...
            pos["current_price"] = price
            pos["market_value"] = pos["quantity"] * price
            if pos["quantity"] <= 0.0001 or pos["market_value"] < 1.0:
    ...
    def calculate_rebalancing_trades(
        ...
        for ticker, target_alloc in target_allocations.items():
            ...
            if abs(diff_value) < min_trade_abs and abs(target_alloc - current_allocs.get(ticker, 0)) < 0.02:
                continue
